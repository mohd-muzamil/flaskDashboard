# This Script is to query the data filter-> "attribute":"Location: and restructure to csv format for easier processing....
#
#
#
#
...
def process_location():
    ...
    for database in databases:
      ...
      dirName = os.path.join("../data/", database)
...
#          ios attributes
        ...
        #   attributes = ["Lock state", "Call", "Brightness", "Accelerometer", "Gyroscope", "Sleep_Noise"]    ########uncomment this line and change value if single attribute data is required. Make sure its a list. commenting this will fetch data for all the attributes########
          attributes = ["powerState", "calls__callDate_callDurationS_callType_phoneNumberHash", "accelerometer_m_s2__x_y_z", "gyroscope_rad_s__x_y_z"]

          for attribute in attributes:
            #things to change filename, participants list, filtes
            filename =  os.path.join(dirName, attribute.replace(" ", "_") + "_android.csv")  ########change this if data needs to be stored with a different filename#########
            ...
            if query_cnt == 0:
              print(f"Query does not fetch any documents for collection[{i+1}/{collection_count}]:{collection} and attribute:{attribute}")
            else:
