...
def write_output(line, filename):
    """
    function to write header line into output files
    """
    # print(line, file=open(os.path.join(outputDataPath, logFile), "a"))
    f = open(filename, "a")
    f.write(line)
    f.close()
...
def process_lockstate_ios_data(inputDataPath, outputDataPath, removeOutputFileFlag, headerWriteFlag):
    ...
    ### Lockstate IOS
    """
    print(f"Processing Lockstate IOS data from '{outputDataPath}'...", file=open(os.path.join(outputDataPath, logFile), "a"))
    ...
    for participantId in (filteredParticipantIds):
        ...
        for date in dates:
            try:
                ...
                if not headerWriteFlag:
                    line = "|".join(lockstate_participantId_date.columns.tolist())
                    write_output(line, lockstateOutputFilename)
                    headerWriteFlag = True

                # append the data to file
                lockstate_participantId_date.to_csv(os.path.join(outputDataPath, lockstateOutputFilename), mode='a', sep="|", header=False, index=False)
...
def process_call_ios_data(inputDataPath, outputDataPath, removeOutputFileFlag, headerWriteFlag):
    ...
    ### Call IOS
    """
    print(f"Processing Call IOS data from '{outputDataPath}'...", file=open(os.path.join(outputDataPath, logFile), "a"))
    ...
    for participantId in (filteredParticipantIds):
        ...
        for date in dates:
            try:
                ...
                if not headerWriteFlag:
                    line = "|".join(call_participantId_date.columns.tolist())
                    write_output(line)
                    headerWriteFlag = True

                # append the data to file
                call_participantId_date.to_csv(os.path.join(outputDataPath, callOutputFilename), mode='a', sep="|", header=False, index=False)
...
def process_brightness_ios_data(inputDataPath, outputDataPath, removeOutputFileFlag, headerWriteFlag):
    ...
    #### Brightness IOS data
    """
    print(f"Processing Brightness IOS data from '{outputDataPath}'...", file=open(os.path.join(outputDataPath, logFile), "a"))
    ...
    for participantId in (filteredParticipantIds):
        ...
        for date in dates:
            try:
                ...
                if not headerWriteFlag:
                    line = "|".join(brightnessAgg.columns.tolist())
                    write_output(line)
                    headerWriteFlag = True

                # append the data to file
                brightnessAgg.to_csv(os.path.join(outputDataPath, brightnessOutputFilename), mode='a', sep="|", header=False, index=False)
...
def process_accelerometer_ios_data(inputDataPath, outputDataPath, removeOutputFileFlag, headerWriteFlag):
    ...
    ### Accelerometer IOS
    """
    print(f"Processing Accelerometer IOS data from '{outputDataPath}'...", file=open(os.path.join(outputDataPath, logFile), "a"))
    ...
    for participantId in (filteredParticipantIds):
        ...
        for date in dates:
            try:
                ...
                if not headerWriteFlag:
                    line = "|".join(accelerometerAgg.columns.tolist())
                    write_output(line)
                    headerWriteFlag = True
                
                # append the data to file
                accelerometerAgg.to_csv(os.path.join(outputDataPath, accelerometerOutputFilename), mode='a', sep="|", header=False, index=False)
...
def process_gyroscope_ios_data(inputDataPath, outputDataPath, removeOutputFileFlag, headerWriteFlag):
    ...
    ### Gyroscope IOS
    """
    print(f"Processing Gyroscope IOS data from '{outputDataPath}'...", file=open(os.path.join(outputDataPath, logFile), "a"))
    ...
    for participantId in (filteredParticipantIds):
        ...
        for date in dates:
            try:
                ...
                gyroscope_participantId_date["gyro"] = np.sqrt(gyroscope_participantId_date["gyro"]**2 + gyroscope_participantId_date["gyro"]**2 + gyroscope_participantId_date["gyro"]**2)
                ...
                if not headerWriteFlag:
                    line = "|".join(gyroscopeAgg.columns.tolist())
                    write_output(line)
                    headerWriteFlag = True
                
                # append the data to file
                gyroscopeAgg.to_csv(os.path.join(outputDataPath, gyroscopeOutputFilename), mode='a', sep="|", header=False, index=False)
...
def process_sleepnoise_ios_data(inputDataPath, outputDataPath, removeOutputFileFlag, headerWriteFlag):
    ...
    ### Sleepnoise IOS
    """
    print(f"Processing Sleepnoise IOS data from '{outputDataPath}'...", file=open(os.path.join(outputDataPath, logFile), "a"))

    sleepnoiseInputFilename = "Sleep_Noise.csv"
    sleepnoiseOutputFilename = "sleepnoise_processed.csv"
    if removeOutputFileFlag:
    ...
    for participantId in (filteredParticipantIds):
        ...
        for date in dates:
            try:
                ...
                if not headerWriteFlag:
                    line = "|".join(sleepnoiseAgg.columns.tolist())
                    write_output(line)
                    headerWriteFlag = True

                # append the data to file
                sleepnoiseAgg.to_csv(os.path.join(outputDataPath, sleepnoiseOutputFilename), mode='a', sep="|", header=False, index=False)
...
def measure_processing_time(method, inputDataPath, outputDataPath, removeOutputFileFlag, headerWriteFlag):
    try:
        ...
        endTime = time()
        print(f"run time: {round((endTime - strTime)/60, 2)} mins\n", file=open(os.path.join(outputDataPath, logFile), "a"))
    except:
...
if __name__ == "__main__":
    ...
    removeOutputFileFlag = True     #flag used to delete a file if it already exists
    headerWriteFlag = True          #flag used to write header row into the output file
