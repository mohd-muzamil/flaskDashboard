...
def process_user(df_current, i, current_user, n_users, min_stay, roam_dist, print_output=None):
    ...
    Parameters:
    df_current -- Pandas dataframe [[participant, timestamp, latitude, longitude]].
                  Note: Columns "participant" and "timestamp" are set as the index
    i -- Incremental counter
    current_user -- participant of user being processed
...
#             print("Processing user {} of {}.".format(i + 1, n_users))
    ...
    df_user_stops["participant"] = current_user
...
def process_data(df, roam_dist, min_stay, n_jobs=1, print_output=None):
    ...
    Parameters:
    df -- Pandas dataframe [[participant, timestamp, latitude, longitude]].
                  Note: Columns "participant" and "timestamp" are set as the index
    n_users -- Total number of users
    min_stay -- Minimum stay duration in minutes
    roam_dist -- Maximum roaming distance in meters
    ...
    df_stops = Parallel(n_jobs=n_jobs)(delayed(process_user)
